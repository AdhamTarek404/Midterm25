    ...
    <div class="col-md-5">
        <div class="card mt-2">
            <div class="card-header bg-dark text-white text-center">
                <h4 class="mb-0"><i class="bi bi-person-plus me-2"></i>Create Librarian</h4>
            </div>
            <div class="card-body p-4">
                @foreach($errors->all() as $error)
                    <div class="alert alert-danger py-2"><i class="bi bi-exclamation-circle me-1"></i> {{ $error }}</div>
                @endforeach
                <form action="{{ route('store_librarian') }}" method="post">
                    {{ csrf_field() }}
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control" placeholder="librarian@example.com" name="email" required value="{{ old('email') }}">
                    </div>
                    <div class="mb-3">
                    ...
                    <button type="submit" class="btn btn-dark w-100"><i class="bi bi-check-lg me-1"></i> Create Librarian</button>
                </form>
            </div>
        </div>
    </div>
